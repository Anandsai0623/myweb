try{
	...
	stage('Deploy to Dev'){
		...
		sshagent(['tomcat-dev']) {
			sh 'ssh ec2-user@52.193.118.225 rm -rf /opt/tomcat8/webapps/myweb*'
		    sh 'scp target/myweb.war ec2-user@52.193.118.225:/opt/tomcat8/webapps/'
		    sh 'ssh ec2-user@52.193.118.225 sudo service tomcat restart'
		}
